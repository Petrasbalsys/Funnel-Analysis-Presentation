...
FunnelGraph.prototype.getPathMedian = function(i) {
    ...
    // Create starting path that's just a vertical line at the leftmost position
    const startX = X[0];
    const path = `M${startX},${Y[0]} ` +
                `L${startX},${YNext[0]} ` +
                `L${startX},${YNext[0]} ` +
                `L${startX},${Y[0]} Z`;
...
FunnelGraph.prototype.getPathMedian = function(i) {
    ...
    // Always start from exact left edge
    //const startX = 0;    
    
    // Calculate initial y-coordinates for the path
    const y1 = Y[0];
    const y2 = YNext[0];
    
    // Create a thin rectangle at the starting position
    return `M${startX},${y1} L${startX},${y2} L${startX},${y2} L${startX},${y1} Z`;
};
...
FunnelGraph.prototype.drawPaths = function() {
    ...
    const animationEnabled = this.animationEnabled !== false;
    const isVertical = this.direction === 'vertical';
    
    // Animation settings
    const duration = '1.5s'; // Make it faster: '0.5s' or slower: '2s'
    const easing = 'ease-in-out'; // Try: 'ease-in-out','ease-out', 'linear', 'cubic-bezier(0.4, 0, 0.2, 1)'
    const delay = 16; // Delay before setting final shape, decrease for faster transition

    
    
    paths.forEach((path, index) => {
        if (!animationEnabled || isVertical) {
          path.setAttribute('d', definitions[index]);
          return;
        }
        ...
        const startingPath = this.getPathMedian(index);
        path.setAttribute('d', startingPath);
        
        // Add animation styles
        path.style.transition = 'none';
        path.style.transform = 'scaleX(0)';
        ...
        path.style.transition = `transform ${duration} ${easing}`;
        path.style.transform = 'scaleX(1)';
        
        // Set the final path shape
        setTimeout(() => {
            path.setAttribute('d', definitions[index]);
        }, delay);
    });

/*    // Then animate the separator lines
    const labels = this.container.querySelectorAll('.svg-funnel-js__label:not(:first-child)');
    
    // First hide all borders immediately
    ...
    setTimeout(() => {
        labels.forEach((label) => {
            label.style.transition = 'border-left-width 0.5s ease-in-out';
            
            // Small additional delay to ensure transition is registered
            requestAnimationFrame(() => {
                label.style.borderLeftWidth = '2px';
            });
        });
    }, parseFloat(duration) * 1000); // Wait for funnel animation to complete
*/
    // Hide original borders permanently
    if (animationEnabled && !isVertical) {
      const labels = this.container.querySelectorAll('.svg-funnel-js__label:not(:first-child)');
      labels.forEach(label => {
      ...
      // Create and animate SVG separator lines
      const mainPoints = this.getMainAxisPoints();
      const height = this.getHeight();
      
      // Create separator group
      const separatorGroup = document.createElementNS('http://www.w3.org/2000/svg', 'g');
      separatorGroup.classList.add('separator-lines');
      
      // Remove any existing separators
      const existingSeparators = svg.querySelector('.separator-lines');
      if (existingSeparators) {
          existingSeparators.remove();
      }
      ...
      for (let i = 1; i < mainPoints.length - 1; i++) {
          ...
          line.setAttribute('x1', mainPoints[i]);
          line.setAttribute('x2', mainPoints[i]);
          ...
          // Animate after funnel expansion
          setTimeout(() => {
              line.style.transition = 'stroke-dashoffset 1s ease-in-out';
              line.style.strokeDashoffset = '0';
          }, parseFloat(duration) * 1000 + (i * 200)); // Stagger the animations
      } 
    } else {
          // Just set up basic separators without animation
          const labels = this.container.querySelectorAll('.svg-funnel-js__label:not(:first-child)');
          labels.forEach(label => {
            label.style.borderLeft = '2px solid #a9a9a9';
          });
    }
};

const originalDraw = FunnelGraph.prototype.draw;
FunnelGraph.prototype.draw = function(options = {}) {
    // Store animation settings for drawPaths to use
    this.animationEnabled = options.animation !== false;
    this.animationDuration = options.animationDuration ? `${options.animationDuration/1000}s` : '1.5s';
